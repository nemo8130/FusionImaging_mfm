...
%
% [m1 s1 l1]=FusionImaging_mfm (sampfold, datmm) will return the
% mean, standard_deviation and length of the (NxN) matrix containing 
...
fprintf('An Inverse-Square polynomial (F~k/r^2) will be fitted:\n');
...
if (length(x)<4)
    [P,s,mu1]=polyfit(x,ybig,1);
    yy=P(1)*xx+P(2);
else
yy=spline(x,ybig,xx);
[P,s,mu1]=polyfit(x,ybig,3);
yy=(P(1)*xx.^3)+(P(2)*xx.^2)+(P(3)*xx)+P(4);
end

xxx=1./xx;
[pp,ss,mu2]=polyfit(xxx,yy,2);       % Using Centering and scaling 
...
figure
fprintf('Now plotting the polyfit\n')
